...
export default function RoomsManagement() {
    ...
    const [isMounted, setIsMounted] = useState<boolean>(false); // Prevents Next.js hydration error
    ...
    // 1. Load saved data from localStorage when component mounts
    useEffect(() => {
    ...
    // 2. Save data to localStorage every time the 'rooms' array changes
    useEffect(() => {
        if (isMounted) {
            localStorage.setItem('hotel_management_rooms', JSON.stringify(rooms));
        }
    }, [rooms, isMounted]);
    ...
    // Return empty fallback UI or loading indicator during server-render / hydration phase
    if (!isMounted) {
        return <div className="min-h-screen bg-slate-950 text-slate-100 p-5 md:p-10 font-sans" />;
    }

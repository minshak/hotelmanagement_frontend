      ...
      <div className="w-[420px] bg-white/10 backdrop-blur-lg border border-white/20 rounded-3xl p-8 shadow-2xl">
        ...
        <div className="mt-5">
          ...
          <div className="flex items-center bg-black/30 border border-white/10 rounded-xl mt-2 px-3">
            ...
            />
              {showPassword ? (
             <Eye size={18} className="text-gray-400 cursor-pointer" onClick={() => setShowPassword(false)}/>
            ) : (
            <EyeOff size={18} className="text-gray-400 cursor-pointer" onClick={() => setShowPassword(true)}/>
)}
          </div>
        </div>
        ...
        {/* Button */}
        <button onClick={handleLogin} className="w-full mt-6 py-3 rounded-xl bg-white/10 border border-white/20 backdrop-blur-md text-white font-semibold hover:bg-white/20 transition duration-300">
         Authorize Access
        </button>
        {/* Status */}
